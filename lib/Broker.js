...
Broker.prototype.workerDelete = function(workerId, disconnect) {
  ...
  if (worker.service) {
    ...
    for (var i = 0; i < service.workers.length; i++) {
      if (workerId !== service.workers[i]) {
        continue;
      }

      service.workers.splice(i, 1);
      break;
    }
  }
...
Broker.prototype.serviceWorkerAdd = function(srvName, workerId, dispatch) {
  var self = this;
...
Broker.prototype.serviceWorkerSelect = function(srvName) {
  ...
  if (service.workers.length) {
    return service.workers[_.random(0, service.workers.length - 1)];
  }
  return null;
};
