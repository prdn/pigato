...
function Broker(endpoint, conf) {
  ...
  this.conf = {
    heartbeat: 2500,
    dmode: 'load',
    intch: 'inproc://#pigato' + uuid.v4()
  };

  _.extend(this.conf, conf);
  
  this.endpoint = endpoint;
  this.intch = this.conf.intch;
...
Broker.prototype.start = function(cb) {
  ...
  this.name = this.name || 'B' + uuid.v4();
  this.socket = zmq.socket('router');
  this.socket.identity = new Buffer(this.name);
  
  this.pub = zmq.socket('pub');
  this.pub.identity = new Buffer(this.name + '/pub');
  ...
  queue.push(function(next) {
    self.pub.bind(self.intch, function() {
      next();
    });
  });
...
Broker.prototype.stop = function(cb) {
  ...
  if (self.pub) {
    queue.push(function(next) {
      self.pub.unbind(self.intch, function() {
        self.pub.close();
        delete self.pub;
        next();
      });
...
Broker.prototype.onWorker = function(msg) {
  ...
  if (type == MDP.W_READY) {
    ...
    if (!srvName) {
      ...
      self.send([workerId, MDP.WORKER, MDP.W_READY]);
