...
Broker.prototype.onWorker = function(msg) {
  ...
  if (type == MDP.W_REPLY || type == MDP.W_REPLY_PARTIAL || type == MDP.W_REPLY_REJECT) {
    ...
    var ridIndex = worker.rid.indexOf(rid);
    if (ridIndex == -1) {
      debug("B: FATAL from worker '%s' (%s), rid mismatch '%s' not in '%s'", workerId, worker.service, rid, worker.rid);
      this.workerDelete(workerId, true);
      return;
    }
...
Broker.prototype.workerDelete = function(workerId, disconnect) {
  ...
  if (worker.service) {
    ...
    for (var i = 0; i < service.waiting.length; i++) {
      ...
      if(service.waiting.length==0){
        ...
        delete this.services[worker.service];
      }
      break;
    }
  }
  ...
  if (worker.rid) {
    worker.rid.forEach(function(rid, index) {
      self.ctrl.rget(rid, function(err, req) {
        if (req) {
          delete req.workerId;
          self.ctrl.rset(rid, req, function() {
...
Broker.prototype.serviceRequireBest = function(srvName) {
  ...
  var self = this;
  var longestSrvName = Object.keys(self.services).reduce(function(longestSrvName, pSrvName) {
    if (pSrvName[pSrvName.length - 1] == "*" && ("" + srvName).indexOf(pSrvName.slice(0, -1)) == 0) {
      longestSrvName = pSrvName.length > longestSrvName.length ? pSrvName : longestSrvName;
    }
    return longestSrvName;
  }, "");

  if (longestSrvName.length > 0) {
    return this.services[longestSrvName];
  }
  // nothing match , lets do previous behaviour
  return this.serviceRequire(srvName);
...
Broker.prototype.serviceWorkerWait = function(srvName, workerId, dispatch) {
  var self = this;
  var service = this.serviceRequireBest(srvName);
  var worker = this.workers[workerId];
  ...
  if (_.indexOf(service.waiting, workerId) == -1) {
...
// this method previously was disabling a worker. currently its only returning a matching worker
Broker.prototype.serviceWorkerUnwait = function(srvName) {
  var service = this.serviceRequireBest(srvName);
  if (service.waiting.length) {
    var index = Math.floor(Math.random() * service.waiting.length);
    return service.waiting[index];
  }
  return null;
};
...
Broker.prototype.dispatch = function(srvName) {
  ...
  var self = this;
  var lkn = 'dispatch_' + srvName;

  if (this.lock[lkn] === undefined) {
    this.lock[lkn] = 0;
  }
    ...
    self.ctrl.rpop(srvName, function(err, req) {
      ...
      async.auto({
        ...
        handle: ['cache', function(next, res) {
          ...
          worker.rid = worker.rid || [];
          worker.rid.push(req.rid);
