...
function _wsel(service) {
  var self = this;

  switch (this.conf.dmode) {
    case 'rand':
...
Broker.prototype.serviceWorkerSelect = function(srvName) {
  var service = this.serviceRequireBest(srvName);
  return _wsel.call(this, service);
};
...
function _rhandle(worker, srvName, callback) {
  var self = this;

  this.ctrl.rpop( srvName, function(err, req, len) {
    var rcnt = 0;
...
Broker.prototype.dispatch = function(srvName) {
  ...
  if (service.workers.length) {
    ...
    _rhandle.call(this, self.workers[workerId], srvName ,done);
  } else {
    done();
  }
};
