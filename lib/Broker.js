...
Broker.prototype.servicesUpdate = function() {
  ...
  _.each(this.services, function(service, srvName) {
    ...
    _.each(self.services, function(aService, aSrvName) {
      ...
      if (rel) {
        aService.w.push(srvName);
        service.w.push(aSrvName);
      }
    });
  });

  _.each(this.services, function(service, srvName) {
    service.w = _.unique(service.w);
    if (!service.w.length) {
...
Broker.prototype.dispatch = function(srvName) {
  ...
  var workerIds = [];
  for (var s = 0; s < service.w.length; s++) {
    var aService = this.serviceRequire(service.w[s]);
