...
Broker.prototype.requestDel = function(req) {
...
Broker.prototype.serviceWorkerSelect = function(srvName, dmode) {
  ...
  var service = this.services[srvName];
  var sstat = this.ctrl.stat(srvName);

  if (service.workers.length && sstat.rcnt) {
    return { 
      service: srvName, 
  ...
  if (srvName[srvName.length - 1] === '*') {
    if (!service.workers.length) {
      return { service: srvName, workerId: null };
    }
    ...
    _.each(this.services, function(mSrv, mSrvName) {
      ...
      if(mSrvName.indexOf(srvName.slice(0, -1)) === 0) {
        var msstat = self.ctrl.stat(mSrvName);
        if (msstat.rcnt) {
    ...
    var sws = { service: srvName, workerId: null };
...
function _rproc(worker, req, callback) {
  if (_.indexOf(req.rejects, worker.workerId) !== -1) {
    req.attempts++;
    this.ctrl.rset(req);
    this.ctrl.rpush(req, 'r');
    callback(null, req.attempts <= this.conf.rattempts ? 1 : 0, 'rand');
    return;
  }
  ...
  var obj = [
    worker.workerId, MDP.WORKER, MDP.W_REQUEST, req.clientId, req.service, '' 
...
function _rhandle(sws, callback) {
  var self = this;

  var worker = this.workers[sws.workerId];

  this.ctrl.rpop(sws.service, function(err, req, len) {
    var rcnt = 0;
...
Broker.prototype.dispatch = function(srvName, dmode) {
  ...
  function done(err, rcnt, dmode) {
    if (rcnt) {
      self.dispatch(srvName, dmode);
    }
  }

  var sws = self.serviceWorkerSelect(srvName, dmode || this.conf.dmode);
  if (!sws || !sws.service || !sws.workerId) {
    return;
  }

  _rhandle.call(this, sws, done);
};
...
Controller.prototype.init = function() {
  ...
  _.each(this.srq, function(srq, srv) {
    var stat = self.stat(srv);
    var srq = self._srq(srv);
    stat.rcnt += srq.length;
  });
};

Controller.prototype.stat = function(srv) {
  if (this.__stat[srv]) {
    return this.__stat[srv];
  }

  var stat = this.__stat[srv] = { rcnt: 0 };
  return stat;
};
...
Controller.prototype.rset = function(req, callback) {
  this.__reqs[req.rid] = _.clone(req, true);
...
Controller.prototype.rget = function(rid, callback) {
  ...
  setImmediate(function() {
    callback(null, _.clone(self.__reqs[rid], true));
  });
};
...
Controller.prototype.rpush = function(req, dir, callback) {
  ...
  var srq = this._srq(srv);
  var stat = this.stat(srv);
...
Controller.prototype.rdel = function(req) {
  ...
  if (!req.workerId) {
    _.pull(this._srq(req.service), req.rid);
  }
};
